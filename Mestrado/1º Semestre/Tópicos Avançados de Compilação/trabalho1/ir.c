...
void ir_statement(statement statement)
{
	...
	{
		ir_print(statement->u.print_.arg0);
	}
...
void ir_print(expression expression)
{
	...
	else if (expression->type->type == BOOL_)
	{
		printf("\t\tb_print t%d\n", temp);
	}	
}
...
int ir_fun_call(fun_call fun_call)
{
	...
	if (fun_call->arg1->arg0->type->type == INT_)
	{
		printf("\t\tt%d <- i_call @%s, [t%d]\n",temp_t, remove_quotes(fun_call->arg0), temp);
		return temp_t++;
	}
	else if (fun_call->arg1->arg0->type->type == REAL_)
	{
		printf("\t\tfp%d <- r_call @%s, [fp%d]\n",temp_fp, remove_quotes(fun_call->arg0), temp);
		return temp_fp++;
	}
