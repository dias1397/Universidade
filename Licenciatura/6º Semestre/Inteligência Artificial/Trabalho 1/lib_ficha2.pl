...
*/
escreve_solucao(no(Estado, Pai, Op, Custo, Profundidade), stat(NN, LL)) :-
    escreveln('----------'),
    escreveln(custo(Custo)),
    escreveln(profundidade(Profundidade)),
    escreveln('----------'),
    escreveln(nos_espandidos(NN)),
    escreveln(max_lista(LL)),
    escreveln('----------'),
    escreve_accoes(no(Estado, Pai, Op, _, _)).

escreve_accoes(nenhum).
escreve_accoes(no(Estado, Pai, Op, _, _)) :-
    escreve_accoes(Pai),
    write(Op), write(' -> '), write(Estado), nl.
